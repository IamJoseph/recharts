...
class Pie extends Component {
  ...
  static parseCoordinateOfPie = (item, offset) => {
    ...
    const innerRadius = getPercentValue(
      item.props.innerRadius,
      maxPieRadius,
      0
    );
    const outerRadius = getPercentValue(
  ...
  static getComposedData = ({
    ...
    const totalPadingAngle =
      (absDeltaAngle >= 360 ? len : len - 1) * paddingAngle;
    ...
    if (sum > 0) {
      sectors = pieData.map((entry, i) => {
        ...
        const middleRadius =
          (coordinate.innerRadius + coordinate.outerRadius) / 2;
        const tooltipPayload = [
          { name, value: val, payload: entry, percentage: percentage }
        ];
  ...
  renderLabels(sectors) {
    ...
    return (
      <Layer className="recharts-pie-labels">
        {labels}
      </Layer>
    );
  }
    ...
    return sectors.map((entry, i) =>
      ...
      >
        {this.renderSectorItem(
          this.isActiveIndex(i) ? activeShape : null,
          entry
        )}
      </Layer>
    );
